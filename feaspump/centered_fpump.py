...
def feasibility_pump(
    ...
    assert (
        abs(
            sum(
                coeff * np_solution[var_names[var.name]]
                for var, coeff in original_objective.expr.items()
            )
            + original_objective.const
            - original_objective.x
        )
        < 1e-8
    ), "Can't compute objective correctly"
    assert (
        np.allclose(A_eq @ np_center, b_eq) and (A_ub @ np_center <= b_ub + 1e-8).all()
    )
    ...
    for gamma in np.linspace(0, 1, n_samples)[:-1]:
        ...
        assert np.logical_or(
            rounded[binary_indices] == 0, rounded[binary_indices] == 1
        ).all()
        ...
        if inequality_satisfied and equality_satisfied:
            objective_val = (
                sum(
                    coeff * rounded[var_names[var.name]]
                    for var, coeff in original_objective.expr.items()
                )
                + original_objective.const
            )
    ...
    rounded_binary = tuple(
        incumbent_rounding[var_names[name]] for name in binary_variable_names
    )
    ...
    while pass_idx < lp_solves:
        ...
        for gamma in np.linspace(0, 1, n_samples)[:-1]:
            ...
            infinity_norm = np.max(np.abs(rounded - interpolation))
            if infinity_norm < incumbent_norm:
                rounding_tuple = tuple(
                    rounded[var_names[name]] for name in binary_variable_names
                )
                if (
                    use_min_norm
                    and (
                        not do_restarts
                        or rounding_tuple not in visited
                        or visited[rounding_tuple] - original_obj_weight > delta_alpha
                    )
                    and (not do_restarts or rounding_tuple != last_rounded_binary)
                ) or incumbent_rounding is None:
                    incumbent_interpolation = tuple(
                        interpolation[var_names[name]] for name in binary_variable_names
                    )
                    incumbent_rounding = tuple(
                        rounded[var_names[name]] for name in binary_variable_names
                    )
                    incumbent_norm = infinity_norm
